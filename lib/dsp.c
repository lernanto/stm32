...
)
{
    ...
    arm_fill_f32(0.0f, ywx, target_num * feature_num);
    return 1;
}
...
)
{
    ...
    if (1 == target_num)
    {
        ...
        arm_scale_f32(ywx, var / *y, ywx, feature_num);
        arm_add_f32(ywx, x, ywx, feature_num);
        arm_scale_f32(ywx, weight * *y, ywx, feature_num);
    }
    else
    {
        /* 注意 1 - w = w * (1 / w - 1) */
        arm_scale_f32(ywx, var, ywx, target_num * feature_num);

        for (size_t j = 0, base = 0; j < target_num; ++j, base += feature_num)
        {
        ...
        arm_scale_f32(ywx, weight, ywx, target_num * feature_num);
    }
...
)
{
    ...
    if (l2 > 0.0f)
    {
        arm_copy_f32(xwx, param_buf, feature_num * feature_num);
        for (size_t i = 0; i < feature_num * feature_num; i += feature_num + 1)
        {
            param_buf[i] += l2;
        }
        arm_mat_init_f32(&a, feature_num, feature_num, param_buf);
    }
    else
    {
    ...
    /* 求 XWX + l2 * I 的逆 */
    arm_mat_init_f32(&a_inv, feature_num, feature_num, param_inv);
    ...
    else
    {
        ...
        arm_mat_init_f32(&b, target_num, feature_num, (float32_t *)ywx);
        arm_mat_init_f32(&x, target_num, feature_num, coef);
...
)
{
    ...
    else
    {
        ...
        arm_mat_init_f32(&coef_mat, target_num, feature_num, (float32_t *)coef);
...
float32_t dsp_linear_regression_decay(float32_t var, float32_t decay)
{
    return var * decay + 1;
}
